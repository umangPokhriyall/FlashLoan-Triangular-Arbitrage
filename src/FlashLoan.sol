...
contract FlashLoan {
    ...
    // Get amounts out
    uint256[] memory amountsOut = IUniswapV2Router02(PANCAKE_ROUTER)
        .getAmountsOut(_amountIn, path);

    uint256 amountRequired = amountsOut[1];

    // Execute the swap
    uint256[] memory amountsReceived = IUniswapV2Router02(PANCAKE_ROUTER)
    ...
    uint256 amountReceived = amountsReceived[1];
   ...
   function initiateArbitrage(address _busdBorrow, uint _amount) external {
    ...
    function pancakeCall(
        ...
        require(pair == msg.sender, "sender should match the pair");
    ...
    console.log("Trade 3 Coin:", trade3Coin);

        bool profCheck = checkResult(repayAmount, trade3Coin);
        require(profCheck, "Arbitrage not profitable");

        // Pay Myself
        IERC20(BUSD).safeTransfer(myAddress, trade3Coin - repayAmount);
